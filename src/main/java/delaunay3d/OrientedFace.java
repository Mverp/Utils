...
public abstract class OrientedFace implements Iterable<Vertex>
{
	...
	public OrientedFace()
	{
		final Tetrahedron adjacent = getAdjacent();
		this.adjacentVertexOrdinal = adjacent == null ? null : adjacent.ordinalOf(getIncident());
	}
	 ...
	 */
	public boolean flip(final int index, final LinkedList<OrientedFace> ears, final Vertex n)
	{
		if (!isValid())
		{
			return true;
		...
		if (reflexEdges == 0 && isConvex(indexOf(n)) && isLocallyDelaunay(index, n, ears))
		{
			...
			final Tetrahedron[] created = flip2to3();
		...
		else if (reflexEdges == 1)
		{
			...
			final Vertex opposingVertex = getVertex(reflexEdge);
			final Tetrahedron t1 = getIncident().getNeighbor(opposingVertex);
			final Tetrahedron t2 = getAdjacent().getNeighbor(opposingVertex);
	 ...
	 *         performed.
	 */
	public Tetrahedron flip(final Vertex n, final List<OrientedFace> ears)
	{
		if (!isValid())
		{
			return null;
		...
		{
			// Only one face of the opposing tetrahedron is visible
			for (final Tetrahedron t : flip2to3())
			{
				final OrientedFace f = t.getFace(n);
				if (f.hasAdjacent())
				{
					ears.add(f);
				}
		...
		else if (reflexEdges == 1 && !isRegular())
		{
			...
			if (t1 != null && t1 == t2)
			{
				for (final Tetrahedron t : flip3to2(reflexEdge))
				{
					final OrientedFace f = t.getFace(n);
					if (f.hasAdjacent())
					{
	...
	{
		assert this.adjacentVertexOrdinal != null;
		final Tetrahedron incident = getIncident();

		final Vertex opposingVertex = getAdjacentVertex();
		final Vertex incidentVertex = getIncidentVertex();
		final Tetrahedron t0 = new Tetrahedron(getVertex(0), incidentVertex, getVertex(1), opposingVertex);
		final Tetrahedron t1 = new Tetrahedron(getVertex(1), incidentVertex, getVertex(2), opposingVertex);
		final Tetrahedron t2 = new Tetrahedron(getVertex(0), getVertex(2), incidentVertex, opposingVertex);
	 ...
	 */
	public Tetrahedron[] flip3to2(final int reflexEdge)
	{
		assert this.adjacentVertexOrdinal != null;
		final Tetrahedron incident = getIncident();
		final Tetrahedron o2 = getIncident().getNeighbor(getVertex(reflexEdge));
		...
		final Vertex x = getVertex(reflexEdge);
		final Vertex y = getIncidentVertex();
		final Vertex z = getAdjacentVertex();
	...
	{
		if (this.adjacentVertexOrdinal == null)
		{
			return null;
		}
		return getAdjacent().getVertex(this.adjacentVertexOrdinal);
	}
	...
	{
		return this.adjacentVertexOrdinal;
	}
	...
	{
		return this.adjacentVertexOrdinal != null;
	}
	...
	private boolean inSphere(final Vertex query, Vertex b, final Vertex c, final Vertex d)
	{
	...
	private boolean isFlippable3ear(final Vertex n)
	{
		final OrientedFace opposingFace = getIncident().getFace(n);
	...
	private boolean isLocallyDelaunay(final int index, final Vertex v, final LinkedList<OrientedFace> ears)
	{
		final Function<Vertex, Boolean> circumSphere = query -> {
			switch (indexOf(v))
			{
			case 0:
		...
		for (int i = 0; i < ears.size(); i++)
		{
			if (index != i)
			{
				final OrientedFace ear = ears.get(i);
				if (ear != this && ear.isValid())
				{
					for (final Vertex e : ear)
					{
